...
Public Function fLicenceState() As Integer
    ...
    If fFExists(gblSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzVirtualMemoryGauge", "licence", gblSettingsFile)
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Virtual-Memory-Gauge-" & gblCodingEnvironment & "/issues", vbNullString, App.Path, 1)
    End If
...
Public Sub readPrefsPosition()
    ...
    If gblDpiAwareness = "1" Then
        gblPrefsHighDpiXPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblPrefsHighDpiYPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formHighDpiYPosTwips", gblSettingsFile)
    ...
    Else
        gblPrefsLowDpiXPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblPrefsLowDpiYPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPositionAndSize()
    ...
    If widgetPrefs.IsLoaded = True And widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzVirtualMemoryGauge", "formHighDpiXPosTwips", gblPrefsHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzVirtualMemoryGauge", "formHighDpiYPosTwips", gblPrefsHighDpiYPosTwips, gblSettingsFile
        Else
            gblPrefsLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblPrefsLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzVirtualMemoryGauge", "formLowDpiXPosTwips", gblPrefsLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzVirtualMemoryGauge", "formLowDpiYPosTwips", gblPrefsLowDpiYPosTwips, gblSettingsFile
...
Private Sub settingsTimer_Timer()
    ...
    gblUnhide = fGetINISetting("Software\PzVirtualMemoryGauge", "unhide", gblSettingsFile)

    If gblUnhide = "true" Then
        'overlayWidget.Hidden = False
        fGauge.gaugeForm.Visible = True
        sPutINISetting "Software\PzVirtualMemoryGauge", "unhide", vbNullString, gblSettingsFile
    End If
