...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hwnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Virtual-Memory-Gauge-VB6/issues", vbNullString, App.path, 1)
    End If
...
Public Sub setMainTooltips()
   On Error GoTo setMainTooltips_Error
    ...
    If gblEnableTooltips = "1" Then
        ...
        fAlpha.gaugeForm.Widgets("housing/tickbutton").Widget.ToolTip = "Choose smooth movement or regular flicks"
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If gblDpiAwareness = "1" Then
        gblFormHighDpiXPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblFormHighDpiYPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formHighDpiYPosTwips", gblSettingsFile)
...
'        End If
    Else
        gblFormLowDpiXPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblFormLowDpiYPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formLowDpiYPosTwips", gblSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzVirtualMemoryGauge", "formHighDpiXPosTwips", gblFormHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzVirtualMemoryGauge", "formHighDpiYPosTwips", gblFormHighDpiYPosTwips, gblSettingsFile
        Else
            gblFormLowDpiXPosTwips = CStr(panzerPrefs.Left)
            gblFormLowDpiYPosTwips = CStr(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzVirtualMemoryGauge", "formLowDpiXPosTwips", gblFormLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzVirtualMemoryGauge", "formLowDpiYPosTwips", gblFormLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(panzerPrefs.hwnd, "open", thisCommand, "Panzer Virtual Memory Gauge.exe prefs", "", 1)
    Else
