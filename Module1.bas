...
Public Function fLicenceState() As Integer
    ...
    If fFExists(PzGSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzVirtualMemoryGauge", "licence", PzGSettingsFile)
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If PzGDpiAwareness = "1" Then
        PzGFormHighDpiXPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formHighDpiXPosTwips", PzGSettingsFile)
        PzGFormHighDpiYPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formHighDpiYPosTwips", PzGSettingsFile)
...
'        End If
    Else
        PzGFormLowDpiXPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formLowDpiXPosTwips", PzGSettingsFile)
        PzGFormLowDpiYPosTwips = fGetINISetting("Software\PzVirtualMemoryGauge", "formLowDpiYPosTwips", PzGSettingsFile)
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If PzGDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzVirtualMemoryGauge", "formHighDpiXPosTwips", PzGFormHighDpiXPosTwips, PzGSettingsFile
            sPutINISetting "Software\PzVirtualMemoryGauge", "formHighDpiYPosTwips", PzGFormHighDpiYPosTwips, PzGSettingsFile
        Else
            PzGFormLowDpiXPosTwips = CStr(panzerPrefs.Left)
            PzGFormLowDpiYPosTwips = CStr(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzVirtualMemoryGauge", "formLowDpiXPosTwips", PzGFormLowDpiXPosTwips, PzGSettingsFile
            sPutINISetting "Software\PzVirtualMemoryGauge", "formLowDpiYPosTwips", PzGFormLowDpiYPosTwips, PzGSettingsFile
